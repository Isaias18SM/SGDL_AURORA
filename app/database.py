...
# Cambiar a False cuando se requieran datos reales de la BD
...
def obtener_asistencias_simuladas(id_usuario, fecha_inicio, fecha_fin):
    """Datos ficticios únicamente para pruebas."""
    registros = []
    ...
    while fecha_actual <= fecha_fin:
        if fecha_actual.weekday() < 5:  # Lunes a Viernes
            contador += 1
            estado = "Falla" if contador % 8 == 0 else "Presente"
            registros.append({
                "Id_Usuario": id_usuario,
                "Fecha_Requerida": fecha_actual,
                "Estado": estado
            })
...
def obtener_asistencias_aprendiz(id_usuario, fecha_inicio, fecha_fin):
    """
    Obtiene las asistencias de un aprendiz.
    Si SIMULACION_ASISTENCIA es True, retorna datos de prueba ficticios.
    """
    if SIMULACION_ASISTENCIA:
        return obtener_asistencias_simuladas(id_usuario, fecha_inicio, fecha_fin)
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    ...
                    Fecha_Requerida AS fecha_inicio,
                    Estado AS estado
                FROM asistencia
                WHERE Id_Usuario = %s
        ...
        print(f"[DB ERROR - ESTADISTICAS] {e}")
        return []
    finally:
        if conn:
            conn.close()